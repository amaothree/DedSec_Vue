...
<template>
    <div class="xw-evaluate-wrap  animated bounceInLeft" v-if="showScore">
        <div class="xw-evaluate-content">
            ...
            <li v-for="items in ScoreDB.scoreDatas" :key="items.index">
                <span>{{items.scoreTitle}}</span>
                <nav>
                  <a href="javascript:;"  v-for="(item ,index) in items.scoreArr" :key="item.index"
...
.xw-score-list li span{
 ...
 nav a{
      float: left;
      background: black no-repeat -38px 0;
